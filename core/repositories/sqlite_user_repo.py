...
class SqliteUserRepository(AbstractUserRepository):
    ...
    def _row_to_user(self, row: sqlite3.Row) -> Optional[User]:
        ...
        return User(
            ...
            last_electric_fish_time=parse_datetime(row["last_electric_fish_time"])
            ...
            last_wof_play_time=parse_datetime(row["last_wof_play_time"])
            if "last_wof_play_time" in row_keys
            else None,
            wof_last_action_time=parse_datetime(row["wof_last_action_time"])
            ...
            # --- [新功能] 添加骰宝冷却字段的读取 ---
            last_sicbo_time=parse_datetime(row["last_sicbo_time"])
