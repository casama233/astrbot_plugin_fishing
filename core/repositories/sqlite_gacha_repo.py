...
class SqliteGachaRepository(AbstractGachaRepository):
    ...
    def get_pool_by_id(self, pool_id: int) -> Optional[GachaPool]:
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM gacha_pools WHERE gacha_pool_id = ?", (pool_id,)
            )
    ...
    def add_pool_template(self, data: Dict[str, Any]) -> GachaPool:
        ...
        with self._get_connection() as conn:
            ...
            """,
                {
                    ...
                    "is_limited_time": 1
                    if data.get("is_limited_time") in (True, "1", 1, "on")
                    else 0,
            ...
            conn.commit()
        pool = self.get_pool_by_id(int(pool_id)) if pool_id is not None else None
        if pool is None:
            raise RuntimeError("Failed to create gacha pool")
        return pool
    ...
    def delete_pool_template(self, pool_id: int) -> None:
        ...
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM gacha_pools WHERE gacha_pool_id = ?", (pool_id,)
            )
            conn.commit()
        ...
        with self._get_connection() as conn:
            ...
            """,
                (
                    ...
                    original_pool["is_limited_time"]
                    if "is_limited_time" in original_pool.keys()
                    else 0,
                    original_pool["open_until"]
                    if "open_until" in original_pool.keys()
                    else None,
                ),
            )
            ...
            # 复制所有物品
            cursor.execute(
                "SELECT * FROM gacha_pool_items WHERE gacha_pool_id = ?", (pool_id,)
            )
            ...
            if new_pool_id is None:
                raise RuntimeError("Failed to copy gacha pool")
            return int(new_pool_id)
    ...
    def add_item_to_pool(self, pool_id: int, data: Dict[str, Any]) -> GachaPoolItem:
        ...
        with self._get_connection() as conn:
            ...
            conn.commit()
        if item_pool_id is None:
            raise RuntimeError("Failed to create gacha pool item")
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
            ...
            row = cursor.fetchone()
        item = self._row_to_gacha_pool_item(row)
        if item is None:
            raise RuntimeError("Failed to load created gacha pool item")
        return item

    def add_pool_item(self, pool_id: int, data: Dict[str, Any]) -> None:
        self.add_item_to_pool(pool_id, data)
    ...
    def delete_pool_item(self, item_pool_id: int) -> None:
        ...
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM gacha_pool_items WHERE gacha_pool_item_id = ?",
                (item_pool_id,),
            )
            conn.commit()
