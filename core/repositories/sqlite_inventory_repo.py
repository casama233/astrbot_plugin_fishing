...
class SqliteInventoryRepository(AbstractInventoryRepository):
    ...
    def get_fish_inventory(self, user_id: str) -> List[UserFishInventoryItem]:
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT user_id, fish_id, quality_level, quantity FROM user_fish_inventory WHERE user_id = ? AND quantity > 0",
                (user_id,),
            )
    ...
    def clear_fish_inventory(self, user_id: str, rarity: Optional[int] = None) -> None:
        with self._connection_manager.get_connection() as conn:
            ...
            if rarity is None:
                cursor.execute(
                    "DELETE FROM user_fish_inventory WHERE user_id = ?", (user_id,)
                )
            else:
                cursor.execute(
                    """
    ...
    def get_user_bait_inventory(self, user_id: str) -> Dict[int, int]:
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT bait_id, quantity FROM user_bait_inventory WHERE user_id = ?",
                (user_id,),
            )
    ...
    def update_bait_quantity(self, user_id: str, bait_id: int, delta: int) -> None:
        ...
        with self._connection_manager.get_connection() as conn:
            ...
            # 删除数量为0的行，保持数据整洁
            cursor.execute(
                "DELETE FROM user_bait_inventory WHERE user_id = ? AND quantity <= 0",
                (user_id,),
            )
            conn.commit()
    ...
    def update_item_quantity(self, user_id: str, item_id: int, delta: int) -> None:
        ...
        with self._connection_manager.get_connection() as conn:
            ...
            cursor.execute(
                "DELETE FROM user_items WHERE user_id = ? AND quantity <= 0", (user_id,)
            )
            conn.commit()
    ...
    def delete_rod_instance(self, rod_instance_id: int) -> None:
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM user_rods WHERE rod_instance_id = ?", (rod_instance_id,)
            )
            conn.commit()
    ...
    def get_user_accessory_instances(self, user_id: str) -> List[UserAccessoryInstance]:
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM user_accessories WHERE user_id = ?", (user_id,)
            )
    ...
    def delete_accessory_instance(self, accessory_instance_id: int) -> None:
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM user_accessories WHERE accessory_instance_id = ?",
    ...
    def update_fish_quantity(
        ...
        with self._connection_manager.get_connection() as conn:
            ...
            # 删除数量为0的行，保持数据整洁
            cursor.execute(
                "DELETE FROM user_fish_inventory WHERE user_id = ? AND quantity <= 0",
                (user_id,),
            )
            conn.commit()
    ...
    def update_zone_configs(self, zone_id: int, configs: str) -> None:
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE fishing_zones SET configs = ? WHERE id = ?", (configs, zone_id)
            )
            conn.commit()
    ...
    def get_specific_fish_ids_for_zone(self, zone_id: int) -> List[int]:
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT fish_id FROM zone_fish_mapping WHERE zone_id = ?", (zone_id,)
            )
    ...
    def update_specific_fish_for_zone(self, zone_id: int, fish_ids: List[int]) -> None:
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM zone_fish_mapping WHERE zone_id = ?", (zone_id,)
            )
            if fish_ids:
                cursor.executemany(
                    "INSERT INTO zone_fish_mapping (zone_id, fish_id) VALUES (?, ?)",
    ...
    def update_rod_instance(self, rod_instance: UserRodInstance):
        ...
        with self._connection_manager.get_connection() as conn:
            ...
            """,
                (
                    rod_instance.rod_id,
                    rod_instance.is_equipped,
                    rod_instance.current_durability,
                    rod_instance.refine_level,
                    rod_instance.is_locked,
                    rod_instance.rod_instance_id,
                    rod_instance.user_id,
                ),
            )
            conn.commit()
        ...
        with self._connection_manager.get_connection() as conn:
            ...
            """,
                (
                    ...
                    accessory_instance.is_equipped,
                    accessory_instance.refine_level,
                    accessory_instance.is_locked,
                    accessory_instance.accessory_instance_id,
                    accessory_instance.user_id,
                ),
            )
            conn.commit()
    ...
    def get_same_rod_instances(
        self, user_id: int, rod_id: str
    ) -> List[UserRodInstance]:
        """获取用户所有相同类型的钓竿实例"""
        with self._connection_manager.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_same_accessory_instances(
        self, user_id: int, accessory_id: str
    ...
    def clear_aquarium_inventory(
        ...
        with self._connection_manager.get_connection() as conn:
            ...
            if rarity is None:
                cursor.execute(
                    "DELETE FROM user_aquarium WHERE user_id = ?", (user_id,)
                )
            else:
                cursor.execute(
                    """
            ...
            try:
                ...
                if pond_count > 0:
                    ...
                    if deduct_from_pond > 0:
                        ...
                        self.update_fish_quantity(
                            user_id, fish_id, -deduct_from_pond, quality_level
                        )
                ...
                if remaining_qty > 0:
                    ...
                    try:
                        self.update_aquarium_fish_quantity(
                            user_id, fish_id, -remaining_qty, quality_level
                        )
