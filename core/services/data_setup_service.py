...
class DataSetupService:
    ...
    def setup_initial_data(self):
        ...
        if not self.gacha_repo.get_pool_items(1):
            self.gacha_repo.add_pool_item(
                1, {"item_type": "rod", "item_id": 4, "quantity": 1, "weight": 10}
            )  # 星辰钓者
            self.gacha_repo.add_pool_item(
                1, {"item_type": "rod", "item_id": 5, "quantity": 1, "weight": 3}
            )  # 海神之赐
            self.gacha_repo.add_pool_item(
                1, {"item_type": "rod", "item_id": 3, "quantity": 1, "weight": 30}
            )  # 碳素纤维竿
            self.gacha_repo.add_pool_item(
                1, {"item_type": "coins", "item_id": 0, "quantity": 10000, "weight": 57}
            )

        if not self.gacha_repo.get_pool_items(2):
            self.gacha_repo.add_pool_item(
                2, {"item_type": "accessory", "item_id": 4, "quantity": 1, "weight": 5}
            )  # 海洋之心
            self.gacha_repo.add_pool_item(
                2, {"item_type": "accessory", "item_id": 3, "quantity": 1, "weight": 15}
            )  # 丰收号角
            self.gacha_repo.add_pool_item(
                2, {"item_type": "coins", "item_id": 0, "quantity": 20000, "weight": 80}
            )
    ...
    def _ensure_shop1_default_items(self):
        ...
        for rod_data in rod_items:
            ...
            # 添加奖励
            self.shop_repo.add_item_reward(
                item_id,
                {
                    "reward_type": "rod",
    ...
    def _fix_existing_baits(self):
        """
        修正已存在的鱼饵数据（如 is_consumable 字段）。
        这个方法会在每次启动时被调用，确保数据与 initial_data.py 保持一致。
        """
        bait_name_to_is_consumable = {bait[0]: bait[13] for bait in BAIT_DATA}

        try:
            existing_baits = self.item_template_repo.get_all_baits()
            for bait in existing_baits:
                expected_is_consumable = bait_name_to_is_consumable.get(bait.name)
                if expected_is_consumable is not None:
                    actual_is_consumable = getattr(bait, "is_consumable", True)
                    if actual_is_consumable != expected_is_consumable:
                        logger.info(
                            f"修正鱼饵 {bait.name} 的 is_consumable: {actual_is_consumable} -> {expected_is_consumable}"
                        )
                        self.item_template_repo.update_bait_template(
                            bait.bait_id, {"is_consumable": expected_is_consumable}
                        )
        except Exception as e:
            logger.warning(f"修正鱼饵数据时出错: {e}")
