...
def up(cursor: sqlite3.Cursor):
    ...
    # 复制旧表数据到新表
    cursor.execute("""
        INSERT INTO fish_new (fish_id, name, description, rarity, base_value, min_weight, max_weight, icon_url, available_zones)
        SELECT fish_id, name, description, rarity, base_value, min_weight, max_weight, icon_url, available_zones
        FROM fish
    """)
...
def down(cursor: sqlite3.Cursor):
    ...
    cursor.execute("""
        INSERT INTO fish_old (fish_id, name, description, rarity, base_value, min_weight, max_weight, icon_url, available_zones)
        SELECT fish_id, name, description, rarity, base_value, min_weight, max_weight, icon_url, available_zones
        FROM fish
        WHERE rarity <= 5
    """)
