...
def draw_gacha_pool_detail_image(
    pool: Any, probabilities: List[Any]
) -> Image.Image:
    width = 980
    row_h = 44
    header_h = 146
    ...
    draw.text(
        (28, 20),
        f"🎰 {_safe_get(pool, \'name\', \'卡池詳情\')}",
        font=title_font,
        fill=(52, 58, 103),
    )
    draw.text(
        (30, 56),
        f"ID：{_safe_get(pool, \'gacha_pool_id\', \'?\')}",
        font=body_font,
        fill=(76, 86, 128),
    )
    if _safe_get(pool, "cost_premium_currency"):
        draw.text(
            (180, 56),
            f"💎 消耗：{_safe_get(pool, \'cost_premium_currency\')} 高級貨幣 / 次",
            font=body_font,
            fill=(76, 86, 128),
        )
    else:
        draw.text(
            (180, 56),
            f"💰 消耗：{_safe_get(pool, \'cost_coins\', 0)} 金幣 / 次",
            font=body_font,
            fill=(76, 86, 128),
        )
    ...
    draw.text(
        ...
        f"💡 抽卡：/抽卡 {_safe_get(pool, \'gacha_pool_id\', 1)} 十連：/十連 {_safe_get(pool, \'gacha_pool_id\', 1)}",
        font=small_font,
        fill=(74, 92, 134),
    )
