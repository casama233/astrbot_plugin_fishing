...
def draw_backpack_image(user_data: Dict[str, Any], data_dir: str) -> Image.Image:
    ...
    if rods:
        for i, rod in enumerate(rods[:4]):  # 最多顯示4個
            ...
            draw.text((x + 15, card_y + 35), stars, font=small_font, fill=color)
            ...
            if max_dur:
                dur_color = (
                    GAME_COLORS["success"]
                    if cur_dur and cur_dur > max_dur * 0.3
    ...
    if accessories:
        for i, acc in enumerate(accessories[:4]):
            ...
            modifiers = [
                ...
                ("bonus_fish_quantity_modifier", "數量"),
                ("bonus_rare_fish_chance", "稀有"),
                ("bonus_coin_modifier", "金幣"),
            ]
            for key, label in modifiers:
                val = acc.get(key, 1.0)
                if val > 1:
                    draw.text(
                        (x + 15, attr_y),
