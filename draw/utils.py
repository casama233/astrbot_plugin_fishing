...
async def get_user_avatar(
    ...
    try:
        ...
        # 优先使用平台提供的头像URL作为缓存键（兼容Discord）
        cache_key = safe_user_id
        if avatar_url and isinstance(avatar_url, str):
            avatar_url = avatar_url.strip()
            if avatar_url:
                cache_key = hashlib.md5(avatar_url.encode("utf-8")).hexdigest()

        avatar_cache_path = os.path.join(cache_dir, f"{cache_key}_avatar.png")
        ...
        if avatar_image is None:
            # 头像来源优先级：显式avatar_url > QQ头像URL（仅数字ID）
            candidate_urls = []
            if avatar_url:
...
def avatar_postprocess(avatar_image: Image.Image, size: int) -> Image.Image:
    ...
    large_draw.rounded_rectangle(
        [0, 0, large_size, large_size], radius=large_radius, fill=255
    )
...
async def get_fish_icon(
    icon_url: str, data_dir: str, icon_size: int = 60
) -> Optional[Image.Image]:
    """
    下载并处理鱼类图标
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.get(icon_url.strip()) as response:
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"下载图标失败，HTTP状态码: {response.status}, URL: {icon_url}"
                            )
                            return None
            except Exception as e:
                # 如果下载失败，记录日志但不抛出异常
